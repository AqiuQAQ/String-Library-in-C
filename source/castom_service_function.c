...
void check_default_collision(castom_printf_spec *spec)
{
  ...
  if (spec->type != FORMAT_TYPE_FLOAT)
    if (spec->precision != -1 || spec->flags & LEFT)
      spec->flags &= ~(ZEROPAD);
}
                   ...
                   const char *s)
{
  ...
  if (spec->precision >= 0 && spec->precision < lim)
    lim = spec->precision;
                                 ...
                                 castom_printf_spec *spec, va_list args)
{
  ...
  if (spec->flags & PLUS || spec->flags & SPACE || (signed long)num < 0)
    ++lim;
